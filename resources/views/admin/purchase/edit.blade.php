    ...
    <section class="content">
     ...
     <div class="box box-default">
            <div class="box-header with-border">
              <h3 class="box-title">Edit Data Pembelian</h3>
            </div>
     <form role="form" method="POST" action="{{ route('purchase-update',$data->id)}}" enctype="multipart/form-data">
        ...
        @csrf
            <div class="box-body">
              <div class="row">
                <div class="col-xs-4">
                  <label>Tanggal</label>
                  <input type="text" class="form-control" placeholder="Tanggal Pembelian" name="purchase_date" value="{{$data->purchase_date}}" id="datepicker">
                ...
                <input type="text" class="form-control" id="ref_no" value="{{$data->invoice_no}}" readonly>
				<input type="hidden" name="invoice_no" value="{{$no_invoice}}" id="invoice_no">
                </div>
                <div class="col-xs-4">
              ...
              <table id="purchaseInvoice" class="table table-bordered table-hover">
                ...
                @if(!empty($detail))
                 @foreach($detail as $data)
                 	<tr id="rowid{{$data->product_id}}">
                    <td align="center">{{$data->product->name}}<input type="hidden" name="product_id[]" value="{{$data->product_id}}"></td>

				  <td align="center"><input id="qty_{{$data->product_id}}" data-id="{{$data->product_id}}" type="text" class="form-control text-center no_units" value="{{$data->quantity}}" name="quantity[]" data-rate="{{$data->price}}"></td>

 					<td align="center"><input min="0"  type="text" class="form-control text-center unitprice" name="price[]" data-id = "{{$data->product_id}}" id="rate_id_{{$data->product_id}}" value="{{$data->price}}"></td>

                    <td align="center"><input readonly class="form-control text-center amount" size="4" type="text" value="{{$data->quantity * $data->price}}" id="amount_{{$data->product_id}}"></td>

                    <td align="center"><button id="{{$data->product_id}}" class="btn btn-xs btn-danger delete_item"><i class="fa fa-trash"></i></button></td>
                  </tr>
                    <?php
                    $stack[] = $data->product_id;
                  ?>
                  @endforeach
              	<tr class="tableInfos">
                            ...
                            <tr class="tableInfos"><td colspan="4" align="right"><strong>Paid</strong></td><td align="left" colspan="2"><input type='text'  class="form-control" id = "paidTotal" name="paid" onkeyup="sum();"></td></tr>
                             <tr class="tableInfos"><td colspan="4" align="right"><strong>Method</strong></td><td align="left" colspan="2">
							 ...
							 <tr class="tableInfos"><td colspan="4" align="right"><strong>Due</strong></td><td align="left" colspan="2"><input type='text'  class="form-control" id = "payment_due"  readonly></td></tr>
							 @endif
            </tbody>
              </table>
            </div>
            ...
            <div class="box-footer">
                 <button type="submit" class="btn btn-primary pull-right margin">Submit</button>
                <button type="submit" class="btn btn-danger pull-right margin"><a href="{{ route('purchase') }}"></a>Kembali</button>
              </div>
          </div>
              </div>
